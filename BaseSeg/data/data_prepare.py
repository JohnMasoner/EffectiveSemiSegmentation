...
def run_prepare_data(cfg):
    ...
    for data in data_prepare.data_info_unlabel:
        try:
            pool.apply_async(data_prepare.process, (data,))
        except Exception as err:
            traceback.print_exc()
...
class DataPrepare(object):
    def __init__(self, cfg):
        ...
        self.unlabel_db_file = self.db_dir + 'seg_raw_unlabel'
        ...
        self.unlabel_dir = cfg.DATA_PREPARE.TRAIN_UNLABEL_DIR
        ...
        self.data_info = self._read_db(types='train')
        self.data_info_unlabel = self._read_db(types='unlabel')
    ...
    def _read_db(self, types):
        local_data = []
        if types == 'train':
            env = lmdb.open(self.train_db_file, map_size=int(1e9))
        elif types == 'unlabel':
            env = lmdb.open(self.unlabel_db_file, map_size=int(1e9))
        else:
            raise 'Check read db type'
        txn = env.begin()
    ...
    def _create_db_file(self, phase='train'):
        if phase == 'train':
            ...
            db_file_path = self.unlabel_db_file
        else:
        ...
        if phase == 'train':
            ...
            series_ids = read_txt(self.cfg.DATA_PREPARE.UNLABEL_SERIES_IDS_TXT)
        else:
        ...
        for series_id in series_ids:
            if phase == 'train':
                ...
                image_path = os.path.join(self.cfg.DATA_PREPARE.TRAIN_UNLABEL_DIR, series_id + '_0000.nii.gz')
